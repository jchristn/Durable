namespace Test.Sqlite
{
    ...
    /// <summary>
    /// Standalone GroupBy test program that can be run independently
    /// </summary>
    public class StandaloneGroupByTest
    {
        public static async Task RunGroupByTests()
        {
            Console.WriteLine("=== STANDALONE GROUP BY TEST ===");
            
            const string connectionString = "Data Source=StandaloneTest;Mode=Memory;Cache=Shared";
            ...
            try
            {
                ...
                List<Person> testData = new List<Person>
                {
                    ...
                    new Person { FirstName = "Grace", LastName = "Taylor", Age = 26, Email = "grace@test.com", Salary = 55000, Department = "Sales" },
                    new Person { FirstName = "Henry", LastName = "Anderson", Age = 38, Email = "henry@test.com", Salary = 78000, Department = "IT" }
                };
                ...
                Console.WriteLine("\n--- Test 2: GroupBy with WHERE (Age >= 30) ---");
                ...
                decimal maxSalary = await repo.Query().GroupBy(p => p.Department).MaxAsync(p => p.Salary);
                decimal minSalary = await repo.Query().GroupBy(p => p.Department).MinAsync(p => p.Salary);
                ...
                // Test 5: Complex scenario - departments with high average salary
                Console.WriteLine("\n--- Test 5: Complex scenario ---");
                Console.WriteLine("Groups with average salary > 70000:");
                
                IEnumerable<IGrouping<string, Person>> highPayDepts = await repo.Query()
                    .GroupBy(p => p.Department)
                    .ExecuteAsync();
                
                foreach (IGrouping<string, Person> group in highPayDepts)
                {
                    decimal avgDeptSalary = group.Average(p => p.Salary);
                    if (avgDeptSalary > 70000)
                    {
                        Console.WriteLine($"{group.Key}: Avg ${avgDeptSalary:N0} ({group.Count()} people)");
                        foreach (Person person in group.OrderByDescending(p => p.Salary))
                        {
                            Console.WriteLine($"  - {person.FirstName} {person.LastName}: ${person.Salary:N0}");
                        }
                    }
                ...
                // Test 6: Sync methods
                Console.WriteLine("\n--- Test 6: Synchronous methods ---");
                IEnumerable<IGrouping<string, Person>> syncGroups = repo.Query()
                    .Where(p => p.Age < 35)
                    .GroupBy(p => p.Department)
                    .Execute();
                
                Console.WriteLine("Departments with people under 35:");
                foreach (IGrouping<string, Person> group in syncGroups)
                {
                    Console.WriteLine($"{group.Key}: {group.Count()} people");
                }
                
                int syncCount = repo.Query().GroupBy(p => p.Department).Count();
                Console.WriteLine($"Sync total count: {syncCount}");
                
                Console.WriteLine("\n✅ All GroupBy tests completed successfully!");
                Console.WriteLine("\n=== SUMMARY ===");
                Console.WriteLine("✓ Basic GroupBy operations");
                Console.WriteLine("✓ WHERE clause filtering before grouping");
                Console.WriteLine("✓ HAVING clause filtering after grouping");
                Console.WriteLine("✓ Aggregate functions (Count, Sum, Average, Min, Max)");
                Console.WriteLine("✓ Complex grouping scenarios");
                Console.WriteLine("✓ Both sync and async operations");
                Console.WriteLine("✓ Proper IGrouping<TKey, T> results");
            }
            catch (Exception ex)
            {
