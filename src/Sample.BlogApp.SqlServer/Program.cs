namespace Sample.BlogApp.SqlServer
{
    ...
    /// <summary>
    /// Sample blog application demonstrating real-world usage of the Durable ORM with SQL Server.
    ...
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            try
            {
                ...
                SqlServerRepository<Author> authorRepo = new SqlServerRepository<Author>(_ConnectionString);
                SqlServerRepository<BlogPost> postRepo = new SqlServerRepository<BlogPost>(_ConnectionString);
                SqlServerRepository<Comment> commentRepo = new SqlServerRepository<Comment>(_ConnectionString);
        ...
        private static void InitializeDatabase()
        {
            ...
            using (SqlConnection masterConnection = new SqlConnection(masterConnectionString))
            {
                ...
                string createDbSql = @"
                    ...
                    END";
                using (SqlCommand command = new SqlCommand(createDbSql, masterConnection))
                {
            ...
            // Now connect to the BlogApp database and create tables
            ...
            string createAuthorsSql = @"
                ...
                BEGIN
                    ...
                    CREATE INDEX idx_username ON authors(username);
                    CREATE INDEX idx_email ON authors(email);
                END";
            ...
            string createBlogPostsSql = @"
                ...
                BEGIN
                    ...
                    CREATE INDEX idx_is_published ON blog_posts(is_published);
                END";
            ...
            string createCommentsSql = @"
                ...
                BEGIN
                    CREATE TABLE comments (
                        ...
                        commenter_name NVARCHAR(100) NOT NULL,
                        commenter_email NVARCHAR(100) NOT NULL,
            ...
            using (SqlCommand command = new SqlCommand(createAuthorsSql, connection))
            {
                command.ExecuteNonQuery();
            }

            using (SqlCommand command = new SqlCommand(createBlogPostsSql, connection))
            {
                command.ExecuteNonQuery();
            }

            using (SqlCommand command = new SqlCommand(createCommentsSql, connection))
            {
                command.ExecuteNonQuery();
            }
        ...
        private static async Task RunBlogScenarios(
            SqlServerRepository<Author> authorRepo,
            SqlServerRepository<BlogPost> postRepo,
            SqlServerRepository<Comment> commentRepo)
        {
        ...
        /// </summary>
        /// <param name="authorRepo">The author repository.</param>
        private static async Task Scenario1_CreateAuthors(SqlServerRepository<Author> authorRepo)
        {
            ...
            SqlServerRepository<BlogPost> postRepo)
        {
            ...
            Console.WriteLine("\n2. Testing rollback scenario (intentional error with invalid foreign key):");
            ...
            using (ITransaction transaction = await postRepo.BeginTransactionAsync())
            {
                ...
                {
                    Console.WriteLine($"   ✓ Expected error caught (foreign key constraint): {ex.Message.Substring(0, Math.Min(80, ex.Message.Length))}...");
                    Console.WriteLine("   Transaction automatically rolled back due to error");
                }
            }

            int postCountAfter = await postRepo.CountAsync();
            Console.WriteLine($"   ✓ Post count before: {postCountBefore}, after: {postCountAfter} (unchanged - rollback successful!)");
        ...
        /// </summary>
        /// <param name="postRepo">The blog post repository.</param>
        private static async Task Scenario8_BatchOperations(SqlServerRepository<BlogPost> postRepo)
        {
            ...
            SqlServerRepository<Comment> commentRepo)
        {
            ...
            string aggregateSql = @"
                SELECT
                    COUNT(*) as post_count,
                    AVG(CAST(view_count AS FLOAT)) as avg_views,
                    MAX(view_count) as max_views
                FROM blog_posts
                WHERE is_published = 1";

            int postCount = 0;
            await using (System.Data.Common.DbConnection connection = new Microsoft.Data.SqlClient.SqlConnection(authorRepo.Settings.BuildConnectionString()))
            {
        ...
        /// </summary>
        /// <param name="postRepo">The blog post repository.</param>
        private static async Task Scenario11_PaginationAndStreaming(SqlServerRepository<BlogPost> postRepo)
        {
