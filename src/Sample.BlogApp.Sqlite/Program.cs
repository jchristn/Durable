namespace Sample.BlogApp.Sqlite
{
    using System;
    ...
    /// <summary>
    /// Sample blog application demonstrating real-world usage of the Durable ORM with SQLite.
    ...
    class Program
    {
        ...
        private static string _ConnectionString = "Data Source=blog.db";
        ...
        static async Task Main(string[] args)
        {
            ...
            try
            {
                ...
                SqliteRepository<Author> authorRepo = new SqliteRepository<Author>(_ConnectionString);
                SqliteRepository<BlogPost> postRepo = new SqliteRepository<BlogPost>(_ConnectionString);
                SqliteRepository<Comment> commentRepo = new SqliteRepository<Comment>(_ConnectionString);
        ...
        private static void InitializeDatabase()
        {
            ...
            using (SqliteCommand command = new SqliteCommand(createAuthorsSql, connection))
            {
                command.ExecuteNonQuery();
            }

            using (SqliteCommand command = new SqliteCommand(createBlogPostsSql, connection))
            {
                command.ExecuteNonQuery();
            }

            using (SqliteCommand command = new SqliteCommand(createCommentsSql, connection))
            {
                command.ExecuteNonQuery();
            }
        ...
        private static async Task RunBlogScenarios(
            SqliteRepository<Author> authorRepo,
            SqliteRepository<BlogPost> postRepo,
            SqliteRepository<Comment> commentRepo)
        {
        ...
        /// </summary>
        /// <param name="authorRepo">The author repository.</param>
        private static async Task Scenario1_CreateAuthors(SqliteRepository<Author> authorRepo)
        {
        ...
        /// </summary>
        /// <param name="postRepo">The blog post repository.</param>
        private static async Task Scenario8_BatchOperations(SqliteRepository<BlogPost> postRepo)
        {
        ...
        /// </summary>
        /// <param name="postRepo">The blog post repository.</param>
        private static async Task Scenario11_PaginationAndStreaming(SqliteRepository<BlogPost> postRepo)
        {
