...
namespace Durable.SqlServer
{
    ...
    /// <summary>
    /// SQL Server-specific implementation of ITransaction for managing database transactions with savepoint support.
    /// </summary>
    internal class SqlServerRepositoryTransaction : ITransaction
    {
        ...
        /// <summary>
        /// Initializes a new instance of the SqlServerRepositoryTransaction class.
        /// </summary>
        /// <param name="connection">The SQL Server connection for this transaction</param>
        /// <param name="transaction">The SQL Server transaction instance</param>
        /// <param name="connectionFactory">The connection factory for returning connections</param>
        /// <exception cref="ArgumentNullException">Thrown when any parameter is null</exception>
        public SqlServerRepositoryTransaction(SqlConnection connection, SqlTransaction transaction, IConnectionFactory connectionFactory)
        {
        ...
        /// SQL Server uses SAVE TRANSACTION syntax for savepoints.
        /// </summary>
        ...
        public ISavepoint CreateSavepoint(string? name = null)
        {
            ...
            return new SqlServerSavepoint(_Connection, _Transaction, name);
        }
        ...
        public async Task<ISavepoint> CreateSavepointAsync(string? name = null, CancellationToken token = default)
        {
            ...
            // Create savepoint asynchronously using SQL Server syntax
            using SqlCommand command = new SqlCommand($"SAVE TRANSACTION {name}", _Connection, _Transaction);
            await command.ExecuteNonQueryAsync(token).ConfigureAwait(false);

            return new SqlServerSavepoint(_Connection, _Transaction, name, false);
        }
        ...
        {
            if (_Disposed)
                throw new ObjectDisposedException(nameof(SqlServerRepositoryTransaction));
        }
