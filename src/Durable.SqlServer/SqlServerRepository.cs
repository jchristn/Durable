namespace Durable.SqlServer
{
    ...
    public class SqlServerRepository<T> : IRepository<T>, IBatchInsertConfiguration, ISqlCapture, ISqlTrackingConfiguration, IDisposable where T : class, new()
    {
        ...
        /// SQL Server can handle large batches efficiently, default is 1000 rows.
        /// </summary>
        public int MaxRowsPerBatch => _BatchConfig.MaxRowsPerBatch;

        /// <summary>
        /// Gets the maximum number of parameters per INSERT statement.
        /// SQL Server has a high limit for parameters, default is 65535.
        /// </summary>
        ...
        /// SQL Server benefits from prepared statement reuse.
        /// </summary>
        ...
        /// SQL Server has excellent support for multi-row INSERT statements.
        /// </summary>
        ...
        {
            return new SqlServerQueryBuilder<T>(this, transaction);
        }
        ...
        public T Create(T entity, ITransaction? transaction = null)
        {
            ...
            if (hasAutoIncrement)
            {
                // Append SELECT SCOPE_IDENTITY() to the INSERT statement to get the ID in one command
                ...
                if (transaction != null)
                {
                    insertedId = ExecuteScalarWithConnection<object>(transaction.Connection, insertSql, transaction.Transaction, parameters.ToArray());
                }
                else
                {
                    using var connection = _ConnectionFactory.GetConnection();
        ...
        private T UpsertWithConnection(System.Data.Common.DbConnection connection, T entity, System.Data.Common.DbTransaction? transaction)
        {
            ...
            try
            {
                ...
                {
                    object? insertedId = ExecuteScalarWithConnection<object>(connection, "SELECT SCOPE_IDENTITY()", transaction);
                    if (insertedId != null && insertedId != DBNull.Value)
                    {
                        object? convertedId = _DataTypeConverter.ConvertFromDatabase(insertedId, _PrimaryKeyProperty.PropertyType, _PrimaryKeyProperty);
                        _PrimaryKeyProperty.SetValue(entity, convertedId);
        ...
        private async Task<T> UpsertAsyncWithConnection(System.Data.Common.DbConnection connection, T entity, System.Data.Common.DbTransaction? transaction, CancellationToken token)
        {
            ...
            try
            {
                ...
                if (_PrimaryKeyProperty != null && _PrimaryKeyProperty.GetValue(entity) == null)
                {
                    object? insertedId = await ExecuteScalarWithConnectionAsync<object>(connection, "SELECT SCOPE_IDENTITY()", transaction, token).ConfigureAwait(false);
                    if (insertedId != null && insertedId != DBNull.Value)
                    {
                        object? convertedId = _DataTypeConverter.ConvertFromDatabase(insertedId, _PrimaryKeyProperty.PropertyType, _PrimaryKeyProperty);
                        _PrimaryKeyProperty.SetValue(entity, convertedId);
        ...
        private List<T> ExecuteFromSqlWithConnection(System.Data.Common.DbConnection connection, string sql, System.Data.Common.DbTransaction? transaction, params object[] parameters)
        {
            ...
            {
                SqlParameter parameter = new SqlParameter($"@p{i}", parameters[i] ?? DBNull.Value);
                command.Parameters.Add(parameter);
            }
        ...
        /// <summary>
        /// Optimized batch insert implementation for SQL Server using multi-row INSERT syntax.
        /// </summary>
        ...
        private IEnumerable<T> CreateManyOptimized(IList<T> entities, ITransaction? transaction)
        {
            ...
            try
            {
                ...
                Dictionary<int, SqlCommand> preparedCommands = new Dictionary<int, SqlCommand>();

                foreach (IList<T> batch in batches)
                {
                    int batchSize = batch.Count;

                    if (EnablePreparedStatementReuse && preparedCommands.TryGetValue(batchSize, out SqlCommand? preparedCommand))
                    {
                ...
                foreach (SqlCommand preparedCommand in preparedCommands.Values)
                {
        ...
        /// <summary>
        /// Asynchronous optimized batch insert implementation for SQL Server using multi-row INSERT syntax.
        /// </summary>
        /// <param name="entities">The entities to insert</param>
        /// <param name="transaction">Optional transaction to execute within</param>
        /// <param name="token">Cancellation token for the async operation</param>
        ...
        /// <returns>Batches of entities optimized for SQL Server insert performance</returns>
        ...
        /// <summary>
        /// Builds a SQL Server multi-row INSERT command for a batch of entities.
        /// </summary>
        /// <param name="command">The command to configure</param>
        /// <param name="entities">The entities to include in the batch</param>
        private void BuildBatchInsertCommand(SqlCommand command, IList<T> entities)
        {
        ...
        /// <summary>
        /// Adds parameters to a SQL Server command for a batch of entities.
        /// </summary>
        /// <param name="command">The command to add parameters to</param>
        /// <param name="entities">The entities to parameterize</param>
        private void AddParametersForBatch(SqlCommand command, IList<T> entities)
        {
            List<string> columns = GetNonAutoIncrementColumns();
        ...
        /// <summary>
        /// Executes a SQL Server batch insert command synchronously.
        /// </summary>
        /// <param name="command">The command to execute</param>
        /// <param name="entities">The entities being inserted</param>
        private void ExecuteBatchInsert(SqlCommand command, IList<T> entities)
        {
            ...
            try
            {
                ...
                if (_PrimaryKeyProperty != null)
                {
                    ...
                    if (hasAutoIncrement)
                    {
                        ...
                        {
                            object? insertedId = reader.GetValue(0);
                            if (insertedId != null && insertedId != DBNull.Value)
                            {
                                object? convertedId = _DataTypeConverter.ConvertFromDatabase(insertedId, _PrimaryKeyProperty.PropertyType, _PrimaryKeyProperty);
                                _PrimaryKeyProperty.SetValue(entities[entityIndex], convertedId);
                            }
                            entityIndex++;
                        }
                        ...
                        {
                            throw new InvalidOperationException($"Expected to insert {entities.Count} rows, but {entityIndex} IDs were returned");
                        }
        ...
        /// <summary>
        /// Executes a SQL Server batch insert command asynchronously.
        /// </summary>
        /// <param name="command">The command to execute</param>
        /// <param name="entities">The entities being inserted</param>
        /// <param name="token">Cancellation token for the async operation</param>
        private async Task ExecuteBatchInsertAsync(SqlCommand command, IList<T> entities, CancellationToken token)
        {
            // Capture SQL for tracking
            if (_CaptureSql && command != null && !string.IsNullOrEmpty(command.CommandText))
            {
        ...
        /// </summary>
        /// <param name="command">The SQL Server command to process</param>
        /// <returns>SQL string with parameter values substituted</returns>
        private string BuildSqlWithParameters(SqlCommand command)
        {
