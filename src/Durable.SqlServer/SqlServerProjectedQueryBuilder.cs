namespace Durable.SqlServer
{
    ...
    public class SqlServerProjectedQueryBuilder<TEntity, TResult> : IQueryBuilder<TResult>
        ...
        where TResult : class, new()
    {
        ...
        /// <summary>
        /// Initializes a new instance of the SqlServerProjectedQueryBuilder class.
        /// </summary>
        ...
        public SqlServerProjectedQueryBuilder(
            SqlServerRepository<TEntity> repository,
            Expression<Func<TEntity, TResult>> selector,
            SqlServerQueryBuilder<TEntity> sourceQueryBuilder,
        ...
        /// <summary>
        /// Parses the selector expression to build column mappings for projection.
        /// </summary>
        ...
        /// <summary>
        /// Gets the source database column name from an expression.
        /// </summary>
        /// <param name="expression">The expression to analyze.</param>
        /// <returns>The source column name.</returns>
        private string GetSourceColumn(Expression expression)
        {
            ...
            try
            {
                ...
                // wrap it in a CASE expression to return 1/0 for true/false
                if (expression.Type == typeof(bool) && expression is BinaryExpression binaryExpr)
                {
                    ExpressionType nodeType = binaryExpr.NodeType;
                    ...
                    {
                        // Wrap boolean expression in CASE to convert to 1/0
                        result = $"CASE WHEN {result} THEN CAST(1 AS BIT) ELSE CAST(0 AS BIT) END";
                    }
                }
        ...
        /// <summary>
        /// Gets the projected column name from a result expression.
        /// </summary>
        /// <param name="expression">The expression to analyze.</param>
        /// <returns>The projected column name.</returns>
        ...
        /// <summary>
        /// Builds the SQL query for the projection.
        /// </summary>
        /// <returns>The complete SQL query string.</returns>
        private string BuildSqlInternal()
        {
            ...
            // OFFSET/FETCH clause (SQL Server syntax)
            if (_SkipCount.HasValue || _TakeCount.HasValue)
            {
                ...
                {
                    sql.Append($" OFFSET {_SkipCount.Value} ROWS");

                    if (_TakeCount.HasValue)
                    {
                        sql.Append($" FETCH NEXT {_TakeCount.Value} ROWS ONLY");
                    }
                }
        ...
        /// <summary>
        /// Maps database results to projected entities.
        /// </summary>
        /// <param name="reader">The data reader containing results.</param>
        /// <returns>Enumerable of mapped projected entities.</returns>
        private IEnumerable<TResult> MapResults(SqlDataReader reader)
        {
            while (reader.Read())
            {
        ...
        /// <summary>
        /// Asynchronously maps database results to projected entities.
        /// </summary>
        /// <param name="reader">The data reader containing results.</param>
        /// <param name="token">The cancellation token.</param>
        /// <returns>Async enumerable of mapped projected entities.</returns>
        private async IAsyncEnumerable<TResult> MapResultsAsync(SqlDataReader reader, [EnumeratorCancellation] CancellationToken token)
        {
        ...
        /// <summary>
        /// Creates a projected result instance from the current reader position.
        /// </summary>
        /// <param name="reader">The data reader positioned at a result row.</param>
        /// <returns>A mapped projected entity instance.</returns>
        private TResult CreateResultInstance(SqlDataReader reader)
        {
        ...
        /// <summary>
        /// Represents a mapping between source database columns and target projection properties.
        /// </summary>
