namespace Durable.SqlServer
{
    ...
    internal class SqlServerEntityMapper<T> where T : class, new()
    {
        ...
        /// <summary>
        /// Initializes a new instance of the SqlServerEntityMapper class.
        /// </summary>
        ...
        /// <summary>
        /// Maps a SQL Server data reader to a list of entities, handling joined results and navigation properties.
        /// </summary>
        /// <param name="reader">The SQL Server data reader containing the query results</param>
        ...
        public List<T> MapJoinedResults(
            ...
            SqlServerJoinBuilder.SqlServerJoinResult joinResult,
        ...
        /// <summary>
        /// Maps a simple SQL Server data reader to a list of entities without joins or navigation properties.
        ...
        /// <exception cref="ArgumentNullException">Thrown when reader is null</exception>
        /// <exception cref="InvalidOperationException">Thrown when mapping encounters errors</exception>
        public List<T> MapSimpleResults(SqlDataReader reader)
        {
            if (reader == null)
                throw new ArgumentNullException(nameof(reader));
        ...
        /// <summary>
        /// Maps a SQL Server data reader to a list of entities asynchronously, handling joined results and navigation properties.
        /// </summary>
        /// <param name="reader">The SQL Server data reader containing the query results</param>
        /// <param name="joinResult">Join metadata for mapping related entities</param>
        /// <param name="includes">Navigation property information for Include operations</param>
        ...
        /// <summary>
        /// Maps a simple SQL Server data reader to a list of entities asynchronously without joins or navigation properties.
        /// Provides enhanced type conversion compared to basic mapping implementations.
        /// </summary>
        /// <param name="reader">The SQL Server data reader containing the query results</param>
        /// <param name="cancellationToken">Cancellation token for the async operation</param>
        ...
        private T MapPrimaryEntity(SqlDataReader reader, Dictionary<object, T> existingEntities)
        {
            T entity = MapSingleEntity(reader, _BaseColumnMappings);
        ...
        private T MapSingleEntity(SqlDataReader reader, Dictionary<string, PropertyInfo> columnMappings)
        {
            T entity = new T();
            ...
            SqlServerJoinBuilder.SqlServerJoinResult joinResult)
        {
            ...
            List<SqlServerJoinBuilder.SqlServerColumnMapping> mappings = joinResult.ColumnMappingsByAlias[include.JoinAlias];
            object entity = Activator.CreateInstance(include.RelatedEntityType)!;

            bool hasAnyValue = false;

            foreach (SqlServerJoinBuilder.SqlServerColumnMapping mapping in mappings)
            {
                try
                {
