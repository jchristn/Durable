namespace Test.SqlServer
{
    ...
    /// <summary>
    /// Tests for SQL Server table initialization system including database creation, table creation,
    /// validation, and default value providers
    /// </summary>
    public class SqlServerInitializationTests : IDisposable
    {
        ...
        public SqlServerInitializationTests(ITestOutputHelper output)
        {
            _output = output;
            _testDatabaseName = $"durable_init_test_{Guid.NewGuid():N}";
            _connectionString = $"Server=localhost;Database={_testDatabaseName};User Id=sa;Password=YourStrong@Passw0rd;TrustServerCertificate=True;";
            _createdDatabases = new List<string>();

            // Check if SQL Server is available
            try
            {
                using SqlConnection connection = new SqlConnection("Server=localhost;User Id=sa;Password=YourStrong@Passw0rd;TrustServerCertificate=True;");
                connection.Open();
                _output.WriteLine("✓ SQL Server is available");
            }
            catch (Exception ex)
            {
                _skipTests = true;
                _output.WriteLine($"⚠️  SQL Server not available - tests will be skipped: {ex.Message}");
            }
        }
        ...
        public void CreateDatabaseIfNotExists_CreatesNewDatabase()
        {
            ...
            string connString = $"Server=localhost;Database={dbName};User Id=sa;Password=YourStrong@Passw0rd;TrustServerCertificate=True;";

            SqlServerRepositorySettings settings = SqlServerRepositorySettings.Parse(connString);
            SqlServerRepository<Product> repository = new SqlServerRepository<Product>(settings);
        ...
        public void InitializeTable_CreatesNewTable()
        {
            ...
            connection.Open();
            using SqlCommand command = connection.CreateCommand();
            command.CommandText = "SELECT name FROM sys.tables WHERE name = 'test_products'";
            object? result = command.ExecuteScalar();
        ...
        public void InitializeTable_WithForeignKey_CreatesRelatedTables()
        {
            ...
            SqlServerRepositorySettings settings = SqlServerRepositorySettings.Parse(connString);
            SqlServerRepository<Order> repository = new SqlServerRepository<Order>(settings);
            _createdDatabases.Add(dbName);
            ...
            command1.CommandText = "SELECT name FROM sys.tables WHERE name = 'test_products'";
            object? result1 = command1.ExecuteScalar();
            Assert.NotNull(result1);

            using SqlCommand command2 = connection.CreateCommand();
            command2.CommandText = "SELECT name FROM sys.tables WHERE name = 'test_orders'";
        ...
        public void ValidateTable_WarnsAboutExtraColumns()
        {
            ...
            connection.Open();
            using SqlCommand command = connection.CreateCommand();
            command.CommandText = "ALTER TABLE test_products ADD extra_field NVARCHAR(100)";
            command.ExecuteNonQuery();
        ...
        public void DefaultValueProvider_SequentialGuid_SetsValue()
        {
            ...
            SqlServerRepositorySettings settings = SqlServerRepositorySettings.Parse(connString);
            SqlServerRepository<Category> repository = new SqlServerRepository<Category>(settings);
            _createdDatabases.Add(dbName);
        ...
        public void FullWorkflow_InitializeAndUseDatabase()
        {
            ...
            SqlServerRepositorySettings settingsproductRepo = SqlServerRepositorySettings.Parse(connString);
            SqlServerRepository<Product> productRepo = new SqlServerRepository<Product>(settingsproductRepo);
            SqlServerRepositorySettings settingsorderRepo = SqlServerRepositorySettings.Parse(connString);
            SqlServerRepository<Order> orderRepo = new SqlServerRepository<Order>(settingsorderRepo);
            _createdDatabases.Add(dbName);
