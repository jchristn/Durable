namespace Test.SqlServer
{
    ...
    /// <summary>
    /// Comprehensive integration tests for SQL Server concurrency control and conflict resolution including:
    ...
    /// </summary>
    public class SqlServerConcurrencyIntegrationTests : IDisposable
    {
        private readonly ITestOutputHelper _output;
        private readonly string _connectionString;
        private readonly bool _skipTests;

        public SqlServerConcurrencyIntegrationTests(ITestOutputHelper output)
        {
            _output = output;
            _connectionString = "Server=view.homedns.org,1433;Database=durable_concurrency_test;User=sa;Password=P@ssw0rd4Sql;TrustServerCertificate=true;Encrypt=false;";
        ...
        public void BasicConcurrencyTest_CreateAndUpdate_WithVersionColumn()
        {
            ...
            try
            {
                using (SqlServerRepository<AuthorWithVersion> repo = new SqlServerRepository<AuthorWithVersion>(_connectionString))
                {
        ...
        public void ClientWinsResolver_ResolvesConflict()
        {
            ...
            try
            {
                ...
                using (SqlServerRepository<AuthorWithVersion> repo = new SqlServerRepository<AuthorWithVersion>(_connectionString, null, null, resolver))
                {
                    CleanupTestData(repo);
        ...
        public async Task ConcurrentUpdates_HandleConflictsCorrectly()
        {
            ...
            try
            {
                ...
                using (SqlServerRepository<AuthorWithVersion> repo = new SqlServerRepository<AuthorWithVersion>(_connectionString, null, null, resolver))
                {
                    ...
                    {
                        using var repo1 = new SqlServerRepository<AuthorWithVersion>(_connectionString, null, null, resolver);
                    ...
                    {
                        using var repo2 = new SqlServerRepository<AuthorWithVersion>(_connectionString, null, null, resolver);
                    ...
                    {
                        using var repo3 = new SqlServerRepository<AuthorWithVersion>(_connectionString, null, null, resolver);
        ...
        public void NoVersionColumn_BehavesNormally()
        {
            ...
            try
            {
                using (SqlServerRepository<AuthorWithoutVersion> repo = new SqlServerRepository<AuthorWithoutVersion>(_connectionString))
                {
                    // Ensure table exists first
                    EnsureTableExistsWithoutVersion(repo);
        ...
        private void CleanupTestData(SqlServerRepository<AuthorWithVersion> repo)
        {
            try
            {
                // Ensure table exists first
        ...
        private void EnsureTableExists(SqlServerRepository<AuthorWithVersion> repo)
        {
            try
            {
            ...
            {
                _output.WriteLine($"Note: Table creation attempt: {ex.Message}");
            }
        }

        private void EnsureTableExistsWithoutVersion(SqlServerRepository<AuthorWithoutVersion> repo)
        {
            try
            {
        ...
        public void Dispose()
        {
            if (!_skipTests)
            {
                try
                {
                    using var repo = new SqlServerRepository<AuthorWithVersion>(_connectionString);
                    CleanupTestData(repo);
                }
                catch (Exception ex)
                {
