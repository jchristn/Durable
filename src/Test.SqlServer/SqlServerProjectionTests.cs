namespace Test.SqlServer
{
    ...
    /// </summary>
    public class SqlServerProjectionTests : IDisposable
    {
        #region Private-Members

        private const string TestConnectionString = "Server=view.homedns.org,1433;Database=durable_projection_test;User=sa;Password=P@ssw0rd4Sql;TrustServerCertificate=true;Encrypt=false;";
        ...
        /// </summary>
        public SqlServerProjectionTests()
        {
            lock (_TestLock)
            {
        ...
        public async Task TestProjectionSqlGeneration()
        {
            ...
            Assert.Contains("OFFSET", sql); // SQL Server uses OFFSET/FETCH instead of LIMIT
        ...
        private void SetupTestDatabase()
        {
            try
            {
                ...
                Console.WriteLine("✅ SQL Server projection tests enabled - database connection successful");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"⚠️  SQL Server projection tests disabled - could not connect to database: {ex.Message}");
                Console.WriteLine("To enable SQL Server projection tests:");
