namespace Test.SqlServer
{
    ...
    /// <summary>
    /// Comprehensive tests for SQL Server entity relationships and complex models including:
    ...
    /// </summary>
    public class SqlServerEntityRelationshipTests : IDisposable
    {
        ...
        private readonly SqlServerRepository<Category> _CategoryRepository;
        private readonly SqlServerRepository<Company> _CompanyRepository;
        private readonly SqlServerRepository<AuthorCategory> _AuthorCategoryRepository;

        public SqlServerEntityRelationshipTests(ITestOutputHelper output)
        {
            _Output = output;
            _ConnectionString = "Server=view.homedns.org,1433;Database=durable_test;User=sa;Password=P@ssw0rd4Sql;TrustServerCertificate=true;Encrypt=false;";
            ...
            try
            {
                ...
                _BookRepository = new SqlServerRepository<Book>(_ConnectionString);
                _CategoryRepository = new SqlServerRepository<Category>(_ConnectionString);
                _CompanyRepository = new SqlServerRepository<Company>(_ConnectionString);
                _AuthorCategoryRepository = new SqlServerRepository<AuthorCategory>(_ConnectionString);

                _Output.WriteLine("SQL Server entity relationship tests initialized successfully");
            }
            catch (Exception ex)
            {
                _SkipTests = true;
        ...
        public async Task CascadingOperations_RelationshipConsistency_MaintainsDataIntegrity()
        {
            ...
            // Test manual cascade deletion (since SQL Server may not have FK CASCADE configured)
        ...
        private async Task SetupDatabaseAsync()
        {
            ...
            await _CompanyRepository.ExecuteSqlAsync("IF OBJECT_ID('dbo.author_categories', 'U') IS NOT NULL DROP TABLE author_categories");
            await _CompanyRepository.ExecuteSqlAsync("IF OBJECT_ID('dbo.books', 'U') IS NOT NULL DROP TABLE books");
            await _CompanyRepository.ExecuteSqlAsync("IF OBJECT_ID('dbo.authors', 'U') IS NOT NULL DROP TABLE authors");
            await _CompanyRepository.ExecuteSqlAsync("IF OBJECT_ID('dbo.categories', 'U') IS NOT NULL DROP TABLE categories");
            await _CompanyRepository.ExecuteSqlAsync("IF OBJECT_ID('dbo.companies', 'U') IS NOT NULL DROP TABLE companies");
            ...
            await _AuthorRepository.ExecuteSqlAsync(@"
                CREATE TABLE authors (
                    ...
                    name VARCHAR(100) NOT NULL,
                    company_id INT NULL,
                    CONSTRAINT fk_authors_company_id FOREIGN KEY (company_id) REFERENCES companies(id)
                )");
            ...
            await _BookRepository.ExecuteSqlAsync(@"
                CREATE TABLE books (
                    ...
                    CONSTRAINT fk_books_publisher_id FOREIGN KEY (publisher_id) REFERENCES companies(id)
                )");
            ...
            await _AuthorCategoryRepository.ExecuteSqlAsync(@"
                CREATE TABLE author_categories (
                    ...
                    CONSTRAINT uk_author_category UNIQUE (author_id, category_id),
                    CONSTRAINT fk_author_categories_author_id FOREIGN KEY (author_id) REFERENCES authors(id),
                    CONSTRAINT fk_author_categories_category_id FOREIGN KEY (category_id) REFERENCES categories(id)
                )");
        ...
        private async Task SeedTestDataAsync()
        {
            ...
            {
                Title = "SQL Server Advanced Techniques",
                AuthorId = author1.Id,
