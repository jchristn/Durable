namespace Test.Postgres
{
    ...
    /// </summary>
    public class PostgresComplexExpressionTests : IDisposable
    {

        #region Private-Members

        private const string TestConnectionString = "Host=localhost;Database=durable_expression_test;Username=test_user;Password=test_password;";
        ...
        /// </summary>
        public PostgresComplexExpressionTests()
        {
            lock (_TestLock)
            {
        ...
        public async Task TestStringFunctions()
        {
            ...
            Assert.True(containsList.Count >= 5);
        ...
        public async Task TestNullHandling()
        {
            ...
            Assert.True(notNullList.Count >= 8);
        ...
        public async Task TestComplexExpressionSqlGeneration()
        {
            ...
            List<Person> resultList = results.ToList();
            ...
            bool hasInOrOr = sql.Contains("IN") || sql.Contains("OR") || sql.Contains("= ANY");
        ...
        private void SetupTestDatabase()
        {
            try
            {
                ...
                Console.WriteLine("✅ PostgreSQL complex expression tests enabled - database connection successful");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"⚠️  PostgreSQL complex expression tests disabled - could not connect to database: {ex.Message}");
                Console.WriteLine("To enable PostgreSQL complex expression tests:");
        ...
        private void CreateTables()
        {
            ...
            _PersonRepository.ExecuteSql("CREATE INDEX idx_first_name ON people(first)");
            _PersonRepository.ExecuteSql("CREATE INDEX idx_last_name ON people(last)");
            _PersonRepository.ExecuteSql("CREATE INDEX idx_email ON people(email)");
            _PersonRepository.ExecuteSql("CREATE INDEX idx_dept_age ON people(department, age)");
            _PersonRepository.ExecuteSql("CREATE INDEX idx_age_salary ON people(age, salary)");
