namespace Test.Postgres
{
    ...
    /// <summary>
    /// Comprehensive data type conversion and mapping tests for PostgreSQL including:
    ...
    /// </summary>
    public class PostgresDataTypeConverterTests : IDisposable
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="PostgresDataTypeConverterTests"/> class.
        /// </summary>
        /// <param name="output">The test output helper for logging test results.</param>
        public PostgresDataTypeConverterTests(ITestOutputHelper output)
        {
            _output = output;
            _connectionString = "Host=localhost;Database=durable_datatype_test;Username=test_user;Password=test_password;";
        ...
        public async Task DateTimeHandling_VariousPrecisions_HandlesDateTimeTypes()
        {
            ...
            var testCases = new[]
            {
                ...
                new
                {
                    ...
                    CreatedDate = DateTime.MinValue.AddYears(1900), // PostgreSQL doesn't support year 1
                    UpdatedDate = (DateTimeOffset?)null,
                    Duration = TimeSpan.FromTicks(10) // 10 ticks = 1 microsecond (PostgreSQL TIME precision limit)
                }
            };
        ...
        /// </summary>
        private async Task CreateTableAsync(PostgresRepository<ComplexEntity> repository)
        {
            ...
            await repository.ExecuteSqlAsync(@"
                CREATE INDEX IF NOT EXISTS idx_status ON complex_entities(status)");

            await repository.ExecuteSqlAsync(@"
                CREATE INDEX IF NOT EXISTS idx_created_date ON complex_entities(created_date)");

            await repository.ExecuteSqlAsync(@"
                CREATE INDEX IF NOT EXISTS idx_unique_id ON complex_entities(unique_id)");

            // Update existing table schema if needed (for duration column)
            try
            {
                await repository.ExecuteSqlAsync("ALTER TABLE complex_entities ALTER COLUMN duration TYPE TEXT;");
                _output.WriteLine("✓ Updated duration column to TEXT");
            }
            catch (Exception ex)
            {
                _output.WriteLine($"Note: Could not update duration column schema (this is normal for new tables): {ex.Message}");
            }
        }
        ...
        private Task VerifyDataIntegrity(ComplexEntity original, ComplexEntity retrieved)
        {
            ...
            if (timeDiff > 1000) // Allow 1 second difference for PostgreSQL precision
            {
