namespace Test.Postgres
{
    ...
    /// <summary>
    /// Comprehensive tests for Include and Join operations for related entity handling in PostgreSQL.
    /// Tests loading navigation properties, nested relationships, and complex entity graphs.
    /// </summary>
    public class PostgresIncludeTests : IDisposable
    {
        #region Private-Members

        private const string TestConnectionString = "Host=localhost;Database=durable_include_test;Username=test_user;Password=test_password;";
        ...
        private readonly PostgresRepository<Author> _AuthorRepository = null!;
        private readonly PostgresRepository<Book> _BookRepository = null!;
        private readonly PostgresRepository<Category> _CategoryRepository = null!;
        private readonly PostgresRepository<Company> _CompanyRepository = null!;
        ...
        public PostgresIncludeTests()
        {
            ...
            _CategoryRepository = new PostgresRepository<Category>(TestConnectionString);
        ...
        private void SetupTestDatabase()
        {
            try
            {
                ...
                Console.WriteLine("✅ PostgreSQL include tests enabled - database connection successful");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"⚠️  PostgreSQL include tests disabled - could not connect to database: {ex.Message}");
                Console.WriteLine("To enable PostgreSQL include tests:");
        ...
        private void CreateTables()
        {
            ...
            _CompanyRepository.ExecuteSql("CREATE INDEX idx_companies_industry ON companies(industry)");
            _CompanyRepository.ExecuteSql("CREATE INDEX idx_companies_name ON companies(name)");
            ...
            _AuthorRepository.ExecuteSql("CREATE INDEX idx_authors_company_id ON authors(company_id)");
            _AuthorRepository.ExecuteSql("CREATE INDEX idx_authors_name ON authors(name)");
            ...
            _CategoryRepository.ExecuteSql("CREATE INDEX idx_categories_name ON categories(name)");
            ...
            _BookRepository.ExecuteSql("CREATE INDEX idx_books_author_id ON books(author_id)");
            _BookRepository.ExecuteSql("CREATE INDEX idx_books_publisher_id ON books(publisher_id)");
            _BookRepository.ExecuteSql("CREATE INDEX idx_books_title ON books(title)");
        ...
        private void DropTablesIfExist()
        {
            try
            {
                _BookRepository.ExecuteSql("DROP TABLE IF EXISTS books CASCADE");
                _CategoryRepository.ExecuteSql("DROP TABLE IF EXISTS categories CASCADE");
                _AuthorRepository.ExecuteSql("DROP TABLE IF EXISTS authors CASCADE");
                _CompanyRepository.ExecuteSql("DROP TABLE IF EXISTS companies CASCADE");
            }
            catch
            {
