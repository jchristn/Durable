namespace Test.MySql
{
    ...
    /// <summary>
    /// Tests for MySQL table initialization system including database creation, table creation,
    /// validation, and default value providers
    /// </summary>
    public class MySqlInitializationTests : IDisposable
    {
        ...
        public MySqlInitializationTests(ITestOutputHelper output)
        {
            _output = output;
            _testDatabaseName = $"durable_init_test_{Guid.NewGuid():N}";
            _connectionString = $"Server=localhost;Database={_testDatabaseName};User=test_user;Password=test_password;";
            _createdDatabases = new List<string>();

            // Check if MySQL is available
            try
            {
                using MySqlConnection connection = new MySqlConnection("Server=localhost;User=test_user;Password=test_password;");
                connection.Open();
                _output.WriteLine("✓ MySQL server is available");
            }
            catch (Exception ex)
            {
                _skipTests = true;
                _output.WriteLine($"⚠️  MySQL server not available - tests will be skipped: {ex.Message}");
            }
        }
        ...
        public void CreateDatabaseIfNotExists_CreatesNewDatabase()
        {
            ...
            string connString = $"Server=localhost;Database={dbName};User=test_user;Password=test_password;";

            MySqlRepositorySettings settings = MySqlRepositorySettings.Parse(connString);
            MySqlRepository<Product> repository = new MySqlRepository<Product>(settings);
        ...
        public void InitializeTable_CreatesNewTable()
        {
            ...
            connection.Open();
            using MySqlCommand command = connection.CreateCommand();
            command.CommandText = "SHOW TABLES LIKE 'test_products'";
            object result = command.ExecuteScalar();
        ...
        public void InitializeTable_WithForeignKey_CreatesRelatedTables()
        {
            ...
            MySqlRepositorySettings settings = MySqlRepositorySettings.Parse(connString);
            MySqlRepository<Order> repository = new MySqlRepository<Order>(settings);
            _createdDatabases.Add(dbName);
            ...
            command1.CommandText = "SHOW TABLES LIKE 'test_products'";
            object result1 = command1.ExecuteScalar();
            Assert.NotNull(result1);

            using MySqlCommand command2 = connection.CreateCommand();
            command2.CommandText = "SHOW TABLES LIKE 'test_orders'";
        ...
        public void DefaultValueProvider_SequentialGuid_SetsValue()
        {
            ...
            MySqlRepositorySettings settings = MySqlRepositorySettings.Parse(connString);
            MySqlRepository<Category> repository = new MySqlRepository<Category>(settings);
            _createdDatabases.Add(dbName);
        ...
        public void FullWorkflow_InitializeAndUseDatabase()
        {
            ...
            MySqlRepositorySettings settingsproductRepo = MySqlRepositorySettings.Parse(connString);
            MySqlRepository<Product> productRepo = new MySqlRepository<Product>(settingsproductRepo);
            MySqlRepositorySettings settingsorderRepo = MySqlRepositorySettings.Parse(connString);
            MySqlRepository<Order> orderRepo = new MySqlRepository<Order>(settingsorderRepo);
            _createdDatabases.Add(dbName);
