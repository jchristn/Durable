namespace Test.MySql
{
    ...
    /// and complex statistical reporting functionality in MySQL.
    /// </summary>
    public class MySqlGroupByTests : IDisposable
    {
        #region Private-Members

        private const string TestConnectionString = "Server=localhost;Database=durable_groupby_test;User=test_user;Password=test_password;";
        ...
        private void SetupTestDatabase()
        {
            try
            {
                ...
                Console.WriteLine("✅ MySQL GROUP BY tests enabled - database connection successful");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"⚠️  MySQL GROUP BY tests disabled - could not connect to database: {ex.Message}");
                Console.WriteLine("To enable MySQL GROUP BY tests:");
