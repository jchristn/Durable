...
namespace Durable.Postgres
{
    ...
    /// </summary>
    internal class PostgresRepositoryTransaction : ITransaction
    {
        ...
        public PostgresRepositoryTransaction(NpgsqlConnection connection, NpgsqlTransaction transaction, IConnectionFactory connectionFactory)
        {
        ...
        public ISavepoint CreateSavepoint(string? name = null)
        {
            ...
            return new PostgresSavepoint(_Connection, _Transaction, name);
        }
        ...
        public async Task<ISavepoint> CreateSavepointAsync(string? name = null, CancellationToken token = default)
        {
            ...
            // Create savepoint asynchronously - PostgreSQL uses different syntax than MySQL
            using NpgsqlCommand command = new NpgsqlCommand($"SAVEPOINT \"{name}\"", _Connection, _Transaction);
            await command.ExecuteNonQueryAsync(token).ConfigureAwait(false);

            return new PostgresSavepoint(_Connection, _Transaction, name, false);
        }
        ...
        {
            if (_Disposed)
                throw new ObjectDisposedException(nameof(PostgresRepositoryTransaction));
        }
