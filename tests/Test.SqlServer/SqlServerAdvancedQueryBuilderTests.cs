namespace Test.SqlServer
{
    ...
    /// <summary>
    /// Comprehensive tests for advanced SQL Server query builder features including set operations,
    /// subqueries, CTEs, window functions, and custom SQL fragments.
    /// </summary>
    public class SqlServerAdvancedQueryBuilderTests : IDisposable
    {
        #region Private-Members

        private const string TestConnectionString = "Server=view.homedns.org,1433;Database=durable_advanced_test;User=sa;Password=P@ssw0rd4Sql;TrustServerCertificate=true;Encrypt=false;";
        ...
        /// </summary>
        public SqlServerAdvancedQueryBuilderTests()
        {
            lock (_TestLock)
            {
            ...
            _CategoryRepository = new SqlServerRepository<Category>(TestConnectionString);
        ...
        private void SetupTestDatabase()
        {
            try
            {
                ...
                Console.WriteLine("✅ SQL Server advanced query builder tests enabled - database connection successful");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"⚠️  SQL Server advanced query builder tests disabled - could not connect to database: {ex.Message}");
                Console.WriteLine("To enable SQL Server advanced query builder tests:");
        ...
        private void DropTablesIfExist()
        {
            try
            {
                _BookRepository.ExecuteSql("IF OBJECT_ID('dbo.books', 'U') IS NOT NULL DROP TABLE books");
                _CategoryRepository.ExecuteSql("IF OBJECT_ID('dbo.categories', 'U') IS NOT NULL DROP TABLE categories");
                _AuthorRepository.ExecuteSql("IF OBJECT_ID('dbo.authors', 'U') IS NOT NULL DROP TABLE authors");
                _CompanyRepository.ExecuteSql("IF OBJECT_ID('dbo.companies', 'U') IS NOT NULL DROP TABLE companies");
            }
            catch
            {
