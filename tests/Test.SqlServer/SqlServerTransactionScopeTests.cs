namespace Test.SqlServer
{
    ...
    public class SqlServerTransactionScopeTests : IDisposable
    {
        ...
        public async Task TestConcurrentSavepointCreation()
        {
            ...
            // Skip this test for SQL Server - transactions are not thread-safe and cannot be shared across concurrent tasks
            // SQL Server requires transactions to be used by a single thread at a time
            _output.WriteLine("=== Skipping Concurrent Savepoint Creation ===");
            _output.WriteLine("SQL Server transactions are not thread-safe and cannot be accessed concurrently from multiple threads.");
            _output.WriteLine("This is a fundamental limitation of SQL Server's transaction model.");
            return;

            // The code below is kept for reference but never executed
                    ...
                    tasks.Add(Task.Run(async () =>
                    {
                        try
                        {
                            ...
                            Person person = new Person
                            {
                                ...
                                LastName = "Concurrent",
                                Age = 18 + taskId,
                                Email = $"savepoint{taskId}@concurrenttest.com",
