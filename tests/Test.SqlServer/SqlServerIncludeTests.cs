namespace Test.SqlServer
{
    ...
    /// <summary>
    /// Comprehensive tests for Include and Join operations for related entity handling in SQL Server.
    /// Tests loading navigation properties, nested relationships, and complex entity graphs.
    /// </summary>
    public class SqlServerIncludeTests : IDisposable
    {
        #region Private-Members

        private const string TestConnectionString = "Server=view.homedns.org,1433;Database=durable_include_test;User=sa;Password=P@ssw0rd4Sql;TrustServerCertificate=true;Encrypt=false;";
        ...
        private void SetupTestDatabase()
        {
            try
            {
                ...
                Console.WriteLine("✅ SQL Server include tests enabled - database connection successful");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"⚠️  SQL Server include tests disabled - could not connect to database: {ex.Message}");
                Console.WriteLine("To enable SQL Server include tests:");
